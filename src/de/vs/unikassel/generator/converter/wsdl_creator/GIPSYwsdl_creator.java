...
import de.vs.unikassel.generator.converter.gipsy.wsla_creator.WSLATemplate;
 ...
 *
 */
public class GIPSYwsdl_creator implements Runnable {
	 ...
	 */
	public GIPSYwsdl_creator(Parser parser) {
		this.parser = parser;
	 ...
	 */
	private void createServiceElement(GIPSYService service) {
	 ...
	 * @param serviceName The name of the service.
	 */
	private void createBindingElement(GIPSYService service) {
	 ...
	 * @param serviceName The name of the service.
	 */
	private void createPortTypeElement(GIPSYService service) {
	...
	private Element createSimpleXSDElement(String elementName) {
		...
		if(elementName.contains("Constraints")){
			xsdElement.setAttribute("type", Namespaces.xsdNamespace.getPrefix()+":boolean");
		}
		else {
	 ...
	 */
	private void createMessageAndXSDElements(GIPSYService service) {
	 ...
	 */
	private void createMessageElement(GIPSYService service, boolean requestMessage) {
	...
	private void createNestedElements(Element parentElement, Element wsdlMessageElement, Vector<String> instances, boolean part) {
		...
		if(instances.size() > 1 && Math.random() <= 0.5) {		
			...
			// Create the simple elements and add them to the complex-element.
	 ...
	 * @param service
	 */
	private void createSemanticExtensions(GIPSYService service) {
	...
	public void createWSDLDocument() {
		...
		for(GIPSYService service : this.parser.getGIPSYServices()) {
			...
			serviceqos.setSlaparameternameserviceconstraint("SLAParameterServiceConstraint"+service.getName());
			...
			serviceqos.setConstraints("" + service.getConstraints());
